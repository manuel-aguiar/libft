...
conversion of at least one of argv[i] fails. User input, ac is the max of integers converted.
If user ac = -1, fuction gets the length of av and converts all to integers, if possible.
(av must be null terminated in that case).

the return value is zero if malloc fails or one of the inputs overflows the integer.
In this case, the location where the array would eb placed is set to NULL for safety

returns the size of the array that was created when it is successfull which can be
asigned to a variable for safe array manipulation.
